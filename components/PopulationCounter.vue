...
<template>
  <div class="flex flex-col items-center justify-center space-y-2 max-w-full overflow-hidden"
    ...
    <span class="text-3xl font-semibold flex items-center" :class="growShrink" v-if="growShrink === 'text-green-500'">
      {{ population.toLocaleString('en-US') }}
      <TrendingUp class="w-10 h-10 ml-2" />
    </span>
    <span class="text-3xl font-semibold flex items-center" :class="growShrink" v-else>
      {{ population.toLocaleString('en-US') }}
